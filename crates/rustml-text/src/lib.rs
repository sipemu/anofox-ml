...
impl CountVectorizer {
    pub fn new() -> Self { Self { min_df: 1, max_df_frac: 1.0 } }
    pub fn with_min_df(mut self, m: usize) -> Self { self.min_df = m; self }
    pub fn with_max_df_frac(mut self, f: f64) -> Self { self.max_df_frac = f; self }
    ...
    pub fn fit_transform(&self, docs: &[&str]) -> Result<(Vec<String>, Array2<f64>)> {
        ...
        let mut x = Array2::<f64>::zeros((n, vocab.len()));
...
impl TfidfVectorizer {
    pub fn new() -> Self { Self { min_df: 1, max_df_frac: 1.0, norm_l2: true } }
    ...
    pub fn fit_transform(&self, docs: &[&str]) -> Result<(Vec<String>, Array2<f64>)> {
        ...
        let (vocab, counts) = cv.fit_transform(docs)?;
        let n = counts.nrows();
        let d = counts.ncols();

        // IDF (sklearn smooth_idf=True): log((1 + n) / (1 + df_t)) + 1
        let mut idf = vec![0.0_f64; d];
        for j in 0..d {
            let mut df = 0;
            for i in 0..n {
                if counts[[i, j]] > 0.0 {
            ...
            idf[j] = ((1.0 + n as f64) / (1.0 + df as f64)).ln() + 1.0;
        }

        let mut tfidf = counts.clone();
        for i in 0..n {
            for j in 0..d {
                tfidf[[i, j]] *= idf[j];
            }
...
impl HashingVectorizer {
    pub fn new(n_features: usize) -> Self {
        Self { n_features, alternate_sign: true, norm_l2: true }
    }
    ...
    pub fn transform(&self, docs: &[&str]) -> Array2<f64> {
        ...
        for (i, d) in docs.iter().enumerate() {
            for t in tokenize(d) {
                ...
                let sign = if self.alternate_sign && (h & 1) == 0 { 1.0 } else { -1.0 };
