...
use ndarray::Array1;
use rustml::core::{permutation_importance, Predict, Result};
use rustml::prelude::*;
...
fn test_golden_permutation_importance_ridge_5feat() {
    ...
    assert!(
        ...
        "R² mismatch: rustml={}, sklearn={}",
        r2,
        baseline_r2
    );

    // Wrap as a Predict-only model to feed permutation_importance.
    struct Wrap(rustml::regression::FittedRidgeRegressor);
    ...
    assert_eq!(
        rank, sklearn_rank,
        "rank order disagrees: rustml={:?}, sklearn={:?}, imps={:?}",
    ...
    assert!(
        rel < 0.10,
        "top-feature importance differs by {:.1}%: rustml={}, sklearn={}",
        rel * 100.0,
        ours,
        theirs
    );
